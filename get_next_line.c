...
/*   Updated: 2017/04/04 15:26:54 by rlecart          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

static t_struct		*ft_newlst(t_struct *gnl, int size, const int fd, int wf)
{
	if (wf == 1)
	{
		if (!(gnl = (struct s_struct*)malloc(sizeof(struct s_struct) * size)))
			return (NULL);
		gnl->adr = NULL;
		gnl->head = NULL;
		gnl->keep = NULL;
...
static int			ft_verif(const int fd, t_struct **gnl, t_gsml *g, char **l)
{
...
static int			ft_gsl(int fd, char **l, t_struct **g, t_gsml *gsml)
{
...
static int			ft_gml(t_struct **g, char **t, char **kt, t_struct **s_tmp)
{
